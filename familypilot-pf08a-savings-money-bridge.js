(()=>{
  ...
  function completeAction(state,actionId,input,actorId='member-anna',inputDeps={},at=Date.now()){
    ...
    try{
      ...
      if(sourceLocationId!==destinationLocationId){physicalResult=d.transfers.createTransfer(state,{sourceWalletId:sourceLocationId,destinationWalletId:destinationLocationId,amount,effectiveDate:finite(input?.effectiveDate,at),note:`${action.title} · ${targetGoal.name}`},actorId,at);if(!physicalResult.ok){restore(state,data);return physicalResult}physicalResult.transfer.economicEventId=economicEventId;physicalResult.transfer.actionOccurrenceId=action.id;physicalResult.transfer.purposeGoalId=targetGoal.id}
      const purposeResult=accounts.createTransfer(state,{sourceAccountId:accounts.OPERATING_ACCOUNT_ID,destinationAccountId:`purpose:${targetGoal.id}`,amount,effectiveDate:finite(input?.effectiveDate,at),note:action.title,economicEventId,sourceLocationId,destinationLocationId,reason:'savings_execution'},d.scope?.capitalSnapshot?.(state)?.capital||0,actorId,at);if(!purposeResult.ok){restore(state,data);return purposeResult}
      if(physicalResult?.transfer?.id)for(const event of purposeResult.purposeAllocationEvents||[])event.linkedWalletTransferId=physicalResult.transfer.id;
