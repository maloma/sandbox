...
begin
  ...
  if p_expected_revision is null or p_expected_revision<0 or p_revision is distinct from p_expected_revision+1 then raise exception using errcode='22023',message='revision invalid'; end if;
...
alter table storage.buckets alter column public set default false;
update storage.buckets set public=false,file_size_limit=6291456,allowed_mime_types=array['application/vnd.familypilot.protected-backup-chunk','application/vnd.familypilot.protected-backup-manifest+json'] where id='familypilot-protected-backups';
