...
(function attachFamilyPilotScope(root){
  ...
  function snapshot(state,selected,operations,movements,adjustments,opening,scope,currency){const flow=totals(operations),transferFlow=movementTotals(movements),adjustment=adjustmentTotal(adjustments),change=flow.income+flow.debtInflow+transferFlow.transferInflow-flow.expense-flow.debtOutflow-transferFlow.transferOutflow+adjustment,operationalFunds=opening+change;return{wallet:selected,scope,currency,opening,...flow,...transferFlow,adjustment,change,operationalFunds,liquidCapital:operationalFunds,receivables:0,liabilities:0,netFamilyCapital:operationalFunds,capital:operationalFunds}}
  ...
  function debtPosition(state,walletIds){let receivables=0,liabilities=0;for(const chain of Array.isArray(state?.debtChains)?state.debtChains:[]){if(chain?.status!=='active'||!walletIds.has(chain.walletId))continue;const balance=Number(chain.currentBalance)||0;if(balance>0)receivables+=balance;else liabilities-=balance}return{receivables,liabilities}}
  function capitalSnapshot(state){const selected=activeWallet(state),input=financialTruthInput(state),truthApi=root.FamilyPilotFinancialTruth,personal=isPersonalWallet(selected),scope=personal?'personal':'household',currency=personal?(selected?.nativeCurrency||state?.household?.baseCurrency||'EUR'):(state?.household?.baseCurrency||selected?.nativeCurrency||'EUR');let operations=[],movements=[],adjustments=[],opening=0;if(selected){if(personal){operations=activeOperations(state).filter(operation=>!isTransfer(operation)&&operation.walletId===selected.id);movements=walletMovements(state,selected.id);adjustments=walletAdjustments(state,selected.id);opening=Number(selected.openingBalance)||0}else{operations=householdCapitalOperations(state);movements=householdCapitalMovements(state);const includedIds=new Set(wallets(state).filter(wallet=>!wallet.archivedAt&&wallet.includedInHouseholdCapital===true).map(wallet=>wallet.id));adjustments=activeAdjustments(state).filter(item=>includedIds.has(item.walletId));opening=(Number(state?.household?.openingCapital)||0)+wallets(state).filter(wallet=>wallet.type!=='household_default'&&wallet.includedInHouseholdCapital===true).reduce((sum,wallet)=>sum+(Number(wallet.openingBalance)||0),0)}}const flow=totals(operations),transferFlow=movementTotals(movements),adjustment=adjustmentTotal(adjustments),change=flow.income+flow.debtInflow+transferFlow.transferInflow-flow.expense-flow.debtOutflow-transferFlow.transferOutflow+adjustment;if(typeof truthApi?.financialTruthSnapshot!=='function')return{wallet:selected||null,scope,currency,opening:0,income:0,expense:0,debtInflow:0,debtOutflow:0,transferInflow:0,transferOutflow:0,adjustment:0,change:0,operationalFunds:0,liquidCapital:0,receivables:0,liabilities:0,netFamilyCapital:0,personalNetCapital:null,assets:0,totalLiabilities:0,capital:0,allValued:false,unresolved:[],contributions:[],financialTruthReady:false,financialTruthError:'financial_truth_unavailable'};const truth=truthApi.financialTruthSnapshot(input);if(truth?.ok!==true)return{wallet:selected||null,scope,currency,opening,income:flow.income,expense:flow.expense,debtInflow:flow.debtInflow,debtOutflow:flow.debtOutflow,transferInflow:transferFlow.transferInflow,transferOutflow:transferFlow.transferOutflow,adjustment,change,operationalFunds:0,liquidCapital:0,receivables:0,liabilities:0,netFamilyCapital:0,personalNetCapital:null,assets:0,totalLiabilities:0,capital:0,allValued:false,unresolved:[],contributions:[],financialTruthReady:true,financialTruthError:truth?.error||'financial_truth_failed'};const selfCheck=typeof truthApi.financialTruthSelfCheck==='function'?truthApi.financialTruthSelfCheck(truth):{ok:true};if(selfCheck?.ok!==true)return{wallet:selected||null,scope,currency,opening,income:flow.income,expense:flow.expense,debtInflow:flow.debtInflow,debtOutflow:flow.debtOutflow,transferInflow:transferFlow.transferInflow,transferOutflow:transferFlow.transferOutflow,adjustment,change,operationalFunds:0,liquidCapital:0,receivables:0,liabilities:0,netFamilyCapital:0,personalNetCapital:null,assets:0,totalLiabilities:0,capital:0,allValued:false,unresolved:[],contributions:[],financialTruthReady:true,financialTruthError:selfCheck?.error||'financial_truth_self_check_failed'};let receivables=0,liabilities=0;for(const contribution of truth.contributions){if(contribution?.resolved!==true||contribution.sourceType!=='debt')continue;const baseAmount=Number(contribution.baseAmount);if(contribution.className==='receivable'&&baseAmount>0)receivables+=baseAmount;if(contribution.className==='liability'&&baseAmount<0)liabilities+=Math.abs(baseAmount)}receivables=Math.round(receivables*100)/100;liabilities=Math.round(liabilities*100)/100;const netCapital=personal?truth.personalNetCapital:truth.netFamilyCapital;return{wallet:selected||null,scope,currency:truth.baseCurrency,opening,income:flow.income,expense:flow.expense,debtInflow:flow.debtInflow,debtOutflow:flow.debtOutflow,transferInflow:transferFlow.transferInflow,transferOutflow:transferFlow.transferOutflow,adjustment,change,operationalFunds:truth.operationalFunds,liquidCapital:truth.operationalFunds,receivables,liabilities,netFamilyCapital:netCapital,personalNetCapital:personal?truth.personalNetCapital:null,assets:truth.assets,totalLiabilities:truth.liabilities,capital:netCapital,allValued:truth.allValued,unresolved:truth.unresolved,contributions:truth.contributions,financialTruthReady:true,financialTruthError:null}}
  function capitalBreakdown(state){const snapshot=capitalSnapshot(state),currency=snapshot?.currency||state?.household?.baseCurrency||'EUR',unresolved=Array.isArray(snapshot?.unresolved)?snapshot.unresolved:[];if(snapshot?.financialTruthReady!==true||snapshot?.financialTruthError)return{cash:0,bank:0,investments:0,total:0,reservedPurpose:0,freelyAvailable:0,locations:[],currency,allValued:false,unresolved,financialTruthReady:snapshot?.financialTruthReady,financialTruthError:snapshot?.financialTruthError||'financial_truth_unavailable'};let cash=0,bank=0,investments=0;const locations=[];for(const contribution of Array.isArray(snapshot.contributions)?snapshot.contributions:[]){if(contribution?.sourceType==='wallet'){const wallet=wallets(state).find(item=>item?.id===contribution.sourceId),moneyForm=wallet?.moneyForm==='cash'?'cash':'bank',resolved=contribution.resolved===true&&Number.isFinite(Number(contribution.baseAmount)),balance=resolved?Math.round(Number(contribution.baseAmount)*100)/100:null,nativeAmount=Number(contribution.nativeAmount),nativeBalance=Number.isFinite(nativeAmount)?(contribution.effect==='liability'?-Math.abs(nativeAmount):Math.abs(nativeAmount)):null;if(resolved){if(moneyForm==='cash')cash+=balance;else bank+=balance}locations.push({walletId:contribution.sourceId,name:wallet?.name||contribution.label||'',moneyForm,locationKind:wallet?.locationKind||'bank_current',balance,currency,nativeBalance,nativeCurrency:contribution.nativeCurrency,resolved:contribution.resolved===true,rateToBase:contribution.rateToBase,valuationSource:contribution.valuationSource,valuedAt:contribution.valuedAt,valuationError:contribution.valuationError})}else if(contribution?.sourceType==='investment'&&contribution.resolved===true&&Number.isFinite(Number(contribution.baseAmount)))investments+=Number(contribution.baseAmount)}cash=Math.round(cash*100)/100;bank=Math.round(bank*100)/100;investments=Math.round(investments*100)/100;const total=Math.round((cash+bank+investments)*100)/100,personal=snapshot.scope==='personal',reservedPurpose=personal?0:Math.round((Array.isArray(state?.savingsGoals)?state.savingsGoals:[]).filter(goal=>goal?.status==='active').reduce((sum,goal)=>sum+(Number(goal.savedAmount)||0),0)*100)/100,operationalFunds=Number.isFinite(Number(snapshot.operationalFunds))?Number(snapshot.operationalFunds):0,freelyAvailable=Math.round(Math.max(0,operationalFunds-reservedPurpose)*100)/100;return{cash,bank,investments,total,reservedPurpose,freelyAvailable,locations,currency,allValued:snapshot.allValued===true,unresolved,financialTruthReady:snapshot.financialTruthReady,financialTruthError:snapshot.financialTruthError||null}}
  function financialTruthInput(state){const selected=activeWallet(state),personal=isPersonalWallet(selected),baseCurrency=personal?(selected?.nativeCurrency||state?.household?.baseCurrency||'EUR'):(state?.household?.baseCurrency||selected?.nativeCurrency||'EUR'),scope=personal?'personal':'household',scopeId=personal?selected?.id||null:null;if(!selected)return{baseCurrency,scope:'household',scopeId:null,walletBalances:[],debtPositions:[],investments:[]};const included=personal?[selected]:wallets(state).filter(wallet=>!wallet.archivedAt&&wallet.includedInHouseholdCapital===true),includedIds=new Set(included.map(wallet=>wallet.id)),walletBalances=included.map(wallet=>{let balance=walletCapitalSnapshot(state,wallet.id).operationalFunds;if(!personal&&wallet.type==='household_default')balance=balance-(Number(wallet.openingBalance)||0)+(Number(state?.household?.openingCapital)||0);return{id:wallet.id,name:wallet.name,balance,currency:wallet.nativeCurrency||state?.household?.baseCurrency||'EUR',scope,scopeId,included:true,...(wallet.valuation!==undefined&&wallet.valuation!==null?{valuation:wallet.valuation}:{})}}),walletRows=new Map(walletBalances.map(row=>[row.id,row])),debtPositions=(Array.isArray(state?.debtChains)?state.debtChains:[]).filter(chain=>chain?.status==='active').map(chain=>{const anchorId=chain.scopeWalletId||chain.walletId,anchorWallet=wallets(state).find(wallet=>wallet.id===anchorId),debtPersonal=isPersonalWallet(anchorWallet),debtScope=debtPersonal?'personal':'household',debtScopeId=debtPersonal?anchorWallet.id:null;return{id:chain.id,status:chain.status,balance:chain.currentBalance,currency:chain.currency||anchorWallet?.nativeCurrency||state?.household?.baseCurrency||'EUR',scope:debtScope,scopeId:debtScopeId,...(chain.valuation!==undefined&&chain.valuation!==null?{valuation:chain.valuation}:{})}}),investments=[];for(const account of Array.isArray(state?.investmentAccounts)?state.investmentAccounts:[]){if(account?.status!=='active')continue;const assignment=(Array.isArray(state?.investmentLocationAssignments)?state.investmentLocationAssignments:[]).find(item=>item?.investmentId===account.id&&item?.status!=='inactive'),locationId=assignment?.locationId||defaultWallet(state)?.id,locationWallet=wallets(state).find(wallet=>wallet.id===locationId),includedHere=personal?locationId===selected.id:includedIds.has(locationId);if(!includedHere)continue;const walletRow=walletRows.get(locationId);if(walletRow)walletRow.balance-=(Number(account.bookAmount)||0);investments.push({id:account.id,name:account.name,currentValue:account.currentValue,currency:account.currency||account.nativeCurrency||locationWallet?.nativeCurrency||baseCurrency,status:account.status,scope,scopeId,...(account.valuation!==undefined&&account.valuation!==null?{valuation:account.valuation}:{})})}return{baseCurrency,scope,scopeId,walletBalances,debtPositions,investments}}
...
(function bootstrapFamilyPilotPackages(root){
  ...
  function mount(){ensurePackageMarker();loadScript('familypilot-persistence-runtime.js',()=>{root.__FP_PERSISTENCE_RUNTIME_SCRIPT_LOADED__=true});loadScript('familypilot-financial-truth.js',()=>{root.__FP_FINANCIAL_TRUTH_RUNTIME_READY__=true;const renderAll=root.__FP_RUNTIME__?.getRenderAll?.();if(typeof renderAll==='function')renderAll()});loadScript('familypilot-viewport-anchor.js',()=>loadScript('familypilot-wallet-transfers.js',settleTransferSurfaceWhenBaseRuntimeReady))}
