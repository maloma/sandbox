(function(root,factory){
  ...
  function normalizeChain(raw,at=Date.now()){
    ...
    const rawDueAt=raw?.expectedDueAt,rawDueType=typeof rawDueAt,allowedDueType=rawDueType==='number'||rawDueType==='string';
    const expectedDueAt=rawDueAt==null||(rawDueType==='string'&&rawDueAt.trim()==='')||!allowedDueType?null:asNumber(rawDueAt,null);
  ...
  function setChainExpectedDueAt(state,chainId,expectedDueAt,actorId='member-anna',at=Date.now()){
    ...
    const dueType=typeof expectedDueAt,clearing=expectedDueAt==null||(dueType==='string'&&expectedDueAt.trim()==='');let nextDueAt=null;
    if(!clearing){if(dueType!=='number'&&dueType!=='string')return{ok:false,error:'Ожидаемая дата возврата некорректна.'};nextDueAt=asNumber(expectedDueAt,NaN);if(!Number.isFinite(nextDueAt))return{ok:false,error:'Ожидаемая дата возврата некорректна.'}}
