(()=>{
  ...
  function boot(attempt=0){
    ...
    style.textContent=`
      ...
      .operation-date-display::after{content:'▦';color:var(--blue);font-size:18px}.operation-date-native-source{display:none!important}
      .fp-date-month-head{display:grid;grid-template-columns:46px 1fr 46px;gap:8px;align-items:center;margin-top:14px}.fp-date-month-head button{height:44px;border:1px solid var(--line);border-radius:13px;background:var(--card);color:var(--ink);font-size:24px}.fp-date-month-title{text-align:center;font-weight:950;text-transform:capitalize}
      .fp-date-legend{display:flex;gap:14px;align-items:center;margin-top:12px;color:var(--muted);font-size:11px}.fp-date-legend span{display:inline-flex;gap:6px;align-items:center}.fp-date-legend i{width:12px;height:12px;border-radius:4px;display:inline-block}.fp-date-legend .today i{border:2px solid var(--blue)}.fp-date-legend .selected i{background:var(--green)}
      .fp-date-weekdays,.fp-date-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:5px}.fp-date-weekdays{margin-top:13px;color:var(--muted);font-size:10px;font-weight:900;text-align:center;text-transform:uppercase}.fp-date-grid{margin-top:7px}.fp-date-day{position:relative;min-height:42px;border:2px solid transparent;border-radius:13px;background:transparent;color:var(--ink);font-weight:850}.fp-date-day.other-month{color:color-mix(in srgb,var(--muted) 60%,transparent)}
      .fp-date-day.is-today{border-color:var(--blue);color:var(--blue)}.fp-date-day.is-today::after{content:'';position:absolute;left:50%;bottom:3px;width:5px;height:5px;border-radius:50%;background:var(--blue);transform:translateX(-50%)}
      .fp-date-day.is-selected{background:var(--green);border-color:var(--green);color:#fff}.fp-date-day.is-selected::after{background:#fff}.fp-date-day.is-selected.is-today{box-shadow:0 0 0 3px color-mix(in srgb,var(--blue) 65%,transparent)}
