...
(function attachFamilyPilotScope(root){
  ...
  function capitalSnapshot(state){const selected=activeWallet(state);if(!selected)return{wallet:null,scope:'household',currency:state?.household?.baseCurrency||'EUR',opening:0,income:0,expense:0,debtInflow:0,debtOutflow:0,transferInflow:0,transferOutflow:0,adjustment:0,change:0,operationalFunds:0,liquidCapital:0,receivables:0,liabilities:0,netFamilyCapital:0,capital:0};if(isPersonalWallet(selected)){const result=walletCapitalSnapshot(state,selected.id),position=debtPosition(state,new Set([selected.id])),netFamilyCapital=result.operationalFunds+position.receivables-position.liabilities;return{...result,...position,scope:'personal',netFamilyCapital,capital:netFamilyCapital}}const operations=householdCapitalOperations(state),movements=householdCapitalMovements(state),includedIds=new Set(wallets(state).filter(wallet=>!wallet.archivedAt&&wallet.includedInHouseholdCapital===true).map(wallet=>wallet.id)),adjustments=activeAdjustments(state).filter(item=>includedIds.has(item.walletId)),opening=Number(state?.household?.openingCapital)||0,additionalOpening=wallets(state).filter(wallet=>wallet.type!=='household_default'&&wallet.includedInHouseholdCapital===true).reduce((sum,wallet)=>sum+(Number(wallet.openingBalance)||0),0),result=snapshot(state,selected,operations,movements,adjustments,opening+additionalOpening,'household',state?.household?.baseCurrency||selected.nativeCurrency||'EUR'),position=debtPosition(state,includedIds),netFamilyCapital=result.operationalFunds+position.receivables-position.liabilities;return{...result,...position,netFamilyCapital,capital:netFamilyCapital}}
