(function attachFamilyPilotScope(root){
  ...
  function capitalSnapshot(state){const selected=activeWallet(state);if(!selected)return{wallet:null,scope:'household',currency:state?.household?.baseCurrency||'EUR',opening:0,income:0,expense:0,debtInflow:0,debtOutflow:0,transferInflow:0,transferOutflow:0,change:0,capital:0};if(isPersonalWallet(selected)){const operations=activeOperations(state).filter(operation=>!isTransfer(operation)&&operation.walletId===selected.id),movements=walletMovements(state,selected.id),opening=Number(selected.openingBalance)||0;return snapshot(state,selected,operations,movements,opening,'personal',selected.nativeCurrency||state?.household?.baseCurrency||'EUR')}const operations=householdCapitalOperations(state),movements=householdCapitalMovements(state),opening=Number(state?.household?.openingCapital)||0,additionalOpening=wallets(state).filter(wallet=>wallet.type!=='household_default'&&wallet.includedInHouseholdCapital===true).reduce((sum,wallet)=>sum+(Number(wallet.openingBalance)||0),0);return snapshot(state,selected,operations,movements,opening+additionalOpening,'household',state?.household?.baseCurrency||selected.nativeCurrency||'EUR')}
...
(function bootstrapFamilyPilotPackages(root){
  ...
  function mount(){ensurePackageMarker();loadScript('familypilot-wallet-transfers.js',loadUiWhenBaseRuntimeReady)}
