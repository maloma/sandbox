(function installFamilyPilotNativeSpeechProviderV1(root){
  ...
  const provider=Object.freeze({
    ...
    async recognize(onPartial,onReady){
      ...
      let result;try{result=await host.recognize(text=>{if(typeof onPartial==='function')onPartial(text)},()=>{if(typeof onReady==='function')onReady()})}catch{return failure('speech_recognition_failed')}
    ...
    async cancel(){if(typeof host.cancel!=='function')return false;try{return(await host.cancel())===true}catch{return false}}
  });
