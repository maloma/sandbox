...
const {storage,context,p,b,d}=load('p3a');
...
const resetState=state('before-reset');p.commitState(resetState);const reset=d.prepare(resetState,{action:'reset_application'});assert(reset.ok);assert.strictEqual(reset.plan.backupOfferRequired,true);assert.strictEqual(reset.plan.irreversible,true);assert.strictEqual(d.candidateDescriptor(reset.plan).available,false);assert.strictEqual(reset.plan.summary.candidateStatus,'unavailable');assert.strictEqual(reset.plan.summary.reason,'reset_candidate_builder_unavailable');assert.strictEqual(d.confirm(reset.plan,{action:'irreversible_privacy_erase',acknowledged:true,strong:true}).error,'action_bound_confirmation_required');assert.strictEqual(d.confirm(reset.plan,acknowledgement(reset.plan)).error,'strong_confirmation_required');assert.strictEqual(d.apply(reset.plan,{},resetState).error,'invalid_action_bound_confirmation');
...
const validReset=prepareAndConfirm(d,resetState,{action:'reset_application'});const resetApplyBefore=snapshot(storage);let resetCommitCalls=0;context.FamilyPilotPersistence=new Proxy(p,{get(target,key){if(key==='commitState')return(...args)=>{resetCommitCalls++;return target.commitState(...args)};return Reflect.get(target,key)}});const resetResult=d.apply(validReset.plan,validReset.confirmation,resetState);context.FamilyPilotPersistence=p;assert.strictEqual(resetResult.error,'reset_candidate_builder_unavailable');assert.strictEqual(resetCommitCalls,0,'plan-only reset must not call commitState');assert.strictEqual(snapshot(storage),resetApplyBefore,'valid reset plan causes zero persistence writes');
