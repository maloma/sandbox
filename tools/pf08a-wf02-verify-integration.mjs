...
for(const needle of ['window.__FP_TEST__.transfers','data-open-transfer','walletTransferModal','walletTransferDetailModal','Перевод перемещает деньги между кошельками',"operation.kind==='income'||operation.kind==='expense'"])if(!ui.includes(needle))fail(`Runtime UI contract missing: ${needle}`);
