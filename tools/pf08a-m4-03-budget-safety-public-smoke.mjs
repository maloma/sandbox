...
const expectedMain = process.env.EXPECTED_MAIN_M4_03_BUDGET_SAFE || '370290069984f86b601b580473471ea8988c1826';
const localSmoke = resolve('tools/pf08a-m4-03-budget-designer-browser-smoke-v4.mjs');
...
function checks(packageFiles) {
  return {
    loader: packageFiles.viewportAnchor.includes('familypilot-m4-03-forecast-correction.js')
      && packageFiles.viewportAnchor.includes('familypilot-m4-03-budget-safety-correction.js')
    ...
    forecastCorrection: packageFiles.forecastCorrection.includes('__obligationAmountCorrection')
      && packageFiles.forecastCorrection.includes('expectedAmountOverride==null'),
    budgetSafety: packageFiles.budgetSafety.includes('__budgetSafetyCorrection')
      && packageFiles.budgetSafety.includes('30_day_minimum_operating')
      && packageFiles.budgetSafety.includes('targetLimit'),
...
async function fetchPackage() {
  ...
  throw new Error(`Published protected budget package did not become ready: ${JSON.stringify(last)}`);
}
...
function runSmoke(directory) {
  return new Promise((resolveRun, rejectRun) => {
    ...
    const timer = setTimeout(() => {
      child.kill('SIGKILL');
      rejectRun(new Error(`Protected budget public smoke timed out\n${stderr.slice(-5000)}`));
    }, 180000);
    ...
    child.once('close', code => {
      ...
      else if (!stdout.includes('PF08A_M4_03_BUDGET_DESIGNER_PASS')) rejectRun(new Error(`Budget safety marker missing\n${stdout.slice(-9000)}`));
...
const directory = mkdtempSync(join(tmpdir(), 'pf08a-m4-03-budget-safety-public-'));
try {
  ...
  console.log(JSON.stringify({
    ...
    next_30_day_safe_surplus: true,
    reserve_target_limit: true,
