...
const expectedMain=process.env.EXPECTED_MAIN_M4_06_SOLVER||'ee3ec7c385b3c6a4d92593cde12d05e3dda75fb0';
const localSmoke=resolve('tools/pf08a-m4-06-goal-deficit-solver-browser-smoke.mjs');
...
function checks(packageFiles){return{
  loader:packageFiles.viewportAnchor.includes('familypilot-m4-06-goal-deficit-solver.js')&&packageFiles.viewportAnchor.includes('familypilot-m4-06-goal-deficit-solver-ui.js'),
  domain:packageFiles.solver.includes('solveDeficit')&&packageFiles.solver.includes('solveGoal')&&packageFiles.solver.includes('findRelief')&&packageFiles.solver.includes('actualStateUnchanged'),
  incomeMeaning:packageFiles.solver.includes('Размер дыры в бюджете')&&packageFiles.solver.includes('не обещание увеличить доход'),
  goal:packageFiles.solver.includes('requiredMonthlyContribution')&&packageFiles.solver.includes('feasibilityHorizonDays')&&packageFiles.solver.includes('365'),
  split:packageFiles.solver.includes('splitRelief')&&packageFiles.solverUi.includes('Как разделить расчёт'),
  ui:packageFiles.solverUi.includes('Решить задачу')&&packageFiles.solverUi.includes('Убрать дефицит')&&packageFiles.solverUi.includes('Достичь цели к дате')&&packageFiles.solverUi.includes('Программа не предполагает, что доход можно просто увеличить'),
  foundation:packageFiles.m406.includes('evaluateScenario')&&packageFiles.m406Ui.includes('Реальные финансы не изменяются'),
...
async function fetchPackage(){let last={};for(let attempt=1;attempt<=36;attempt++){
  ...
  await sleep(5000);
}throw new Error(`Published M4-06 solver package did not become ready: ${JSON.stringify(last)}`)}
...
try{
  ...
  console.log(JSON.stringify({status:'PASS',public_url:publicUrl,expected_main:expectedMain,publication_attempts:published.attempt,browser_marker:'PF08A_M4_06_GOAL_DEFICIT_SOLVER_PASS',actual_baseline:true,saved_scenario_source:true,horizons:[30,90],deterministic_relief:true,income_gap_informational:true,expense_reduction_informational:true,combined_split:true,goal_by_date:true,goal_funding_gap:true,actual_state_isolation:true,saved_scenario_isolation:true,no_real_plan_mutation:true,runtime_exceptions:[]},null,2));
