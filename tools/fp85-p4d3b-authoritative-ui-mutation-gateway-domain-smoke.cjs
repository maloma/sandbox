...
function obligationHarness(initial=obligationFixture()){
  const nodes={},ui={close:0,render:0,screen:[],toasts:[],alerts:[]};
  for(const id of ['obligationAmount','obligationWallet','obligationName','obligationDueDate','obligationCadence','obligationCategory','obligationNote','obligationRuleError','obligationRuleSave','obligationPayDate','obligationPayAmount','obligationPayError','obligationPaySave','obligationPostponeDate','obligationPostponeError','obligationPostponeSave','obligationSkipBtn'])nodes[id]={value:'',textContent:'',disabled:false};
  const context={state:clone(initial),obligationEditingRuleId:'',obligationActionOccurrenceId:'',obligationDetailId:'',$:id=>nodes[id]||(nodes[id]={value:'',textContent:'',disabled:false}),now:()=>1700000000000,wallet:id=>context.state.wallets.find(item=>item.id===id),dateFromInput:value=>new Date(`${value}T00:00:00`).getTime(),dateInputValue:value=>new Date(value).toISOString().slice(0,10),close:()=>{ui.close++},renderAll:()=>{ui.render++},showScreen:screen=>{ui.screen.push(screen)},toast:message=>{ui.toasts.push(message)},alert:message=>{ui.alerts.push(message)},confirm:()=>true,console,JSON,Date,Promise,setTimeout,queueMicrotask};
  vm.createContext(context);vm.runInContext(fs.readFileSync(path.join(root,'familypilot-obligations.js'),'utf8'),context);vm.runInContext(`const obligationApi=FamilyPilotObligations;\n${['saveObligationRule','saveObligationPayment','saveObligationPostpone','skipObligationOccurrence'].map(name=>production[name]).join('\n')}`,context);
  return{context,nodes,ui};
}
