...
assert.match(cropContract,/executeReceiptReplacement/,'crop persistence must use the new replacement transaction authority');
assert(cropContract.indexOf('validateCropSelection')<cropContract.indexOf('executeReceiptReplacement'),'validator must reject before the mutation workflow');
assert(cropContract.indexOf('renderValidatedCropCanvas')<cropContract.indexOf('executeReceiptReplacement'),'the exact validated Bnorm crop must be rendered before staging');
