...
assert(cropContract.indexOf('await putReceiptBlob(replacement.storageKey,cropped)')<cropContract.indexOf('operation.receipts=before.map'),'cropped bytes must persist before metadata switches');
assert(cropContract.indexOf('operation.receipts=before.map')<cropContract.indexOf('try{save()}'),'metadata must switch before its durable save');
assert(cropContract.indexOf('try{save()}')<cropContract.indexOf('await deleteReceiptBlob(item.storageKey)'),'the old canonical blob may be deleted only after replacement persistence succeeds');
assert.match(cropContract,/catch\(error\)\{operation\.receipts=before;await deleteReceiptBlob\(replacement\.storageKey\)/,'failed metadata persistence must roll back and retain the old blob');
...
assert.match(index,/Math\.ceil\(size\/1024\)/,'the UI must preserve actual stored attachment size');
