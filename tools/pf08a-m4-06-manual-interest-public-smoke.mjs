...
const expectedMain=process.env.EXPECTED_MAIN_M4_06_INTEREST||'41550d7270e0a989183c06d17aa34bc478dd3849';
const localSmoke=resolve('tools/pf08a-m4-06-manual-interest-browser-smoke.mjs');
...
function checks(packageFiles){return{
  loader:packageFiles.viewportAnchor.includes('familypilot-m4-06-manual-interest.js')&&packageFiles.viewportAnchor.includes('familypilot-m4-06-manual-interest-ui.js'),
  domain:packageFiles.interest.includes('sourceSnapshot')&&packageFiles.interest.includes('calculate')&&packageFiles.interest.includes('saveSimulation')&&packageFiles.interest.includes('isStale'),
  formula:packageFiles.interest.includes('annualRate/12/100')&&packageFiles.interest.includes('contributionTiming')&&packageFiles.interest.includes('end_of_month'),
  baseline:packageFiles.interest.includes('endingWithoutInterest')&&packageFiles.interest.includes('ownMoney')&&packageFiles.interest.includes('interestEarned'),
  scenario:packageFiles.interest.includes('scenarioDelta')&&packageFiles.interest.includes('monthlyAdditionalIncome')===false,
  snapshot:packageFiles.interest.includes('whatIfInterestSimulations')&&packageFiles.interest.includes('sourceFingerprint')&&packageFiles.interest.includes('duplicateSimulation')&&packageFiles.interest.includes('archiveSimulation'),
  noApply:!packageFiles.interest.includes('applyToRealPlan'),
  ui:packageFiles.interestUi.includes('Проценты на накопления')&&packageFiles.interestUi.includes('Свои деньги')&&packageFiles.interestUi.includes('математическая гипотеза')&&!packageFiles.interestUi.includes('Применить'),
  foundation:packageFiles.m406.includes('evaluateScenario')&&packageFiles.solver.includes('solveGoal')&&packageFiles.conversion.includes('prepareScenarioConversion'),
...
async function fetchPackage(){let last={};for(let attempt=1;attempt<=36;attempt++){
  ...
  await sleep(5000);
}throw new Error(`Published M4-06 interest package did not become ready: ${JSON.stringify(last)}`)}
...
try{
  ...
  console.log(JSON.stringify({status:'PASS',public_url:publicUrl,expected_main:expectedMain,publication_attempts:published.attempt,browser_marker:'PF08A_M4_06_MANUAL_INTEREST_PASS',worked_example:true,zero_interest_baseline:true,own_money_separation:true,goal_source:true,custom_source:true,scenario_delta_aggregation:true,generic_scenario_assumptions_ignored:true,target_months:true,high_rate_warning:true,long_term_warning:true,non_finite_rejected:true,saved_snapshot:true,stale_detection:true,duplicate_archive:true,no_apply_to_real_plan:true,financial_state_isolation:true,runtime_exceptions:[]},null,2));
