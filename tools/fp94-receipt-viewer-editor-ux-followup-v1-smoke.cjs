...
const controlIds=['receiptCropAction','receiptShareAction','receiptExportAction','receiptAdjustmentsAction','receiptDoneAction'];
...
assert(!/receiptAutoAction|receiptOpenPdfAction/.test(markup),'Enhance or redundant PDF action remains visible');
assert(/id="receiptAdjustmentsReset"/.test(markup),'Adjustments Reset is missing');
assert(/function resetReceiptManualValues\(\).*const changed=.*brightness=0.*contrast=0/.test(index),'Reset does not clear both tone controls');
assert(/function receiptSessionHasEdit\(\).*crop.*brightness!==0.*contrast!==0/.test(index),'dirty state does not account for independent crop and tone edits');
assert(/receiptAdjustmentsReset'\)\.onclick=.*resetReceiptManualValues\(\).*renderReceiptEditPreview/.test(index),'Reset is not preview-only UI behavior');
assert(!/receiptAutoAction'\)\.onclick|receiptOpenPdfAction'\)\.onclick/.test(index),'removed action retains an event path');
assert(/async function openReceiptPreview\(.*item\.type==='application\/pdf'.*openReceiptPdfDirect\(item\).*return/.test(index),'PDF does not bypass the image viewer');
assert(/async function openReceiptPdfDirect\(item\).*resolveCurrentCanonicalReceiptForAction\(item\).*native_pdf_open_rejected.*Не удалось открыть PDF/.test(index),'PDF direct-open validation/failure boundary missing');
assert(!/receiptPreviewPdf/.test(markup),'PDF object remains in image viewer markup');
assert(/receipt-toolbar.*receipt-tool-dock.*receiptDoneAction/s.test(markup),'Done is not outside the portrait scroll strip');
assert(/receiptToolbarOverflowCue/.test(markup)&&/function syncReceiptToolbarOverflow\(\).*scrollWidth.*scrollLeft/.test(index),'truthful portrait overflow cue missing');
