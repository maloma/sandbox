...
assert.match(adapterSource,/voiceWrap\.hidden=!voiceEnabled/);
assert.match(adapterSource,/\['\+','−','×','÷'\]/);
assert.doesNotMatch(adapterSource,/\['\+','−','×','÷','\(','\)'\]/);
assert.match(adapterSource,/minimumFractionDigits:fractional\?2:0/);
assert.match(adapterSource,/fontSize=length<=8\?'32px'/);
...
assert.strictEqual(resultNode.textContent,'2,40 €'.replace(' ',' '));
