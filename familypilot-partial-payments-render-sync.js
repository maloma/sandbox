  ...
  function boot(attempt=0){
    ...
    function decorateRows(){
      ...
      for(const group of document.querySelectorAll('#obligationList .obligation-date-group')){
        ...
        const plan=group.querySelector('.obligation-day-plan small');
        if(plan)plan.textContent=totals.size?`Запланировать: ${[...totals].map(([code,value])=>money(value,code)).join(' · ')}`:'Всё отмечено';
      }
    }
    ...
    function updatePlanIndicator(){
      ...
      const valid=list=>list.filter(entry=>!hidden[entry.occurrence.id]&&payments.summary(entry.occurrence.id).remaining>EPSILON);
