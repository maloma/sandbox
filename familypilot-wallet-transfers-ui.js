(()=>{
  ...
  try{
    ...
    if(!runtime||!transferApi||!walletManagementApi||window.__FP_WF02_UI__){
      if(!runtime)window.__FP_WF02_UI_ERROR__='FamilyPilot runtime extension bridge is unavailable';
      return;
    }
    window.__FP_WF02_UI__=true;

    const {state,members:MEMBERS,scopeApi,$,esc,isoLocal,open,close,money,formatDateTime,save,showScreen,toast,ensureAccessibleActiveWallet,now}=runtime;
    const renderAll=()=>runtime.renderAll();
    ...
    const baseOperationRow=runtime.getOperationRow();
    runtime.setOperationRow(function(operation){
    ...
    const baseOpenDetail=runtime.getOpenDetail();
    runtime.setOpenDetail(function(id){if(openTransferDetail(id))return;baseOpenDetail(id)});
    const baseAnalyticsPeriodOperations=runtime.getAnalyticsPeriodOperations();
    runtime.setAnalyticsPeriodOperations(function(){return baseAnalyticsPeriodOperations().filter(operation=>operation.kind==='income'||operation.kind==='expense')});

    const baseRenderAll=runtime.getRenderAll();
    runtime.setRenderAll(function(){transferApi.normalizeState(state,now());const result=baseRenderAll();installAction();installWalletEntry();return result});
    ...
    window.__FP_WF02_UI_ERROR__=String(error?.stack||error?.message||error);
    throw error;
  }
})();
